// frontend/app/dashboard/features/youtube-uploader/page.tsx
'use client';
...
import { 
  ...
  History, Trash2, Eye, Clock, Youtube, Music, Video, Hash, Copy
} from 'lucide-react';
...
export default function YouTubeUploaderPage() {
  ...
  const fetchHistory = async () => {
    // Mock data
    setHistory([
      { id: 1, title: 'Video Viral Tips', status: 'success', date: new Date().toISOString(), views: 1234, url: 'https://youtu.be/abc123' },
      { id: 2, title: 'Tutorial AI Content', status: 'success', date: new Date(Date.now() - 86400000).toISOString(), views: 5678, url: 'https://youtu.be/def456' },
      { id: 3, title: 'Behind the Scene', status: 'failed', date: new Date(Date.now() - 172800000).toISOString(), error: 'Invalid video format' },
    ]);
  };

  const handleConnect = () => {
    if (!clientId.trim() || !apiKey.trim()) {
      toast.error(language === 'en' ? 'Please fill Client ID and API Key' : 'Isi Client ID dan API Key');
      return;
    }
    setConnecting(true);
    setTimeout(() => {
      setIsConnected(true);
      setConnecting(false);
      toast.success(language === 'en' ? 'Connected to YouTube API' : 'Terhubung ke YouTube API');
    }, 1500);
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (file.size > 200 * 1024 * 1024) {
        toast.error(language === 'en' ? 'File too large (max 200MB)' : 'File terlalu besar (maks 200MB)');
        return;
      }
      setVideoFile(file);
    }
  ...
  const handleUpload = () => {
    if (!isConnected) {
      toast.error(language === 'en' ? 'Please connect to YouTube first' : 'Hubungkan ke YouTube terlebih dahulu');
      return;
    }
    if (!videoFile) {
      toast.error(language === 'en' ? 'Please select a video file' : 'Pilih file video');
    ...
    const interval = setInterval(() => {
      setUploadProgress(prev => {
        if (prev >= 100) {
          ...
          setUploading(false);
          toast.success(language === 'en' ? 'Video uploaded successfully!' : 'Video berhasil diupload!');
          // Add to history
          const newVideo = {
            id: Date.now(),
            title: title,
  ...
  const t = (key: string) => {
    const translations: any = {
      en: {
        pageTitle: 'AUTOLIVE YOUTUBE UPLOADER',
        subTitle: 'UPLOAD TO YOUTUBE',
        description: 'Upload your videos directly to YouTube with API integration',
        backToDashboard: 'Dashboard',
        logout: 'Logout',
        userName: 'Username',
        role: 'Role',
        activeUntil: 'Active until',
        connection: 'YouTube API Connection',
        clientId: 'Client ID',
        apiKey: 'API Key',
        connectBtn: 'Connect to YouTube',
      ...
      id: {
        pageTitle: 'AUTOLIVE YOUTUBE UPLOADER',
        subTitle: 'UPLOAD KE YOUTUBE',
        description: 'Upload video Anda langsung ke YouTube dengan integrasi API',
        backToDashboard: 'Dashboard',
        logout: 'Keluar',
        userName: 'Nama Pengguna',
        role: 'Hak Akses',
        activeUntil: 'Aktif sampai',
        connection: 'Koneksi YouTube API',
        clientId: 'Client ID',
        apiKey: 'API Key',
        connectBtn: 'Hubungkan ke YouTube',
        ...
        tagsLabel: 'Tag (pisahkan koma)',
        privacyLabel: 'Status Privasi',
        public: 'Publik',
        unlisted: 'Tidak terdaftar',
        private: 'Pribadi',
        uploadBtn: 'Upload ke YouTube',
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      {/* HEADER - Sama seperti halaman viral filter */}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" value={clientId} onChange={(e) => setClientId(e.target.value)} placeholder="xxxxxxxxxxxx-xxxxxxxx.apps.googleusercontent.com" className="w-full p-2 rounded-lg border" disabled={isConnected} />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">{t('apiKey')}</label>
                  <input type="text" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="AIzaSy..." className="w-full p-2 rounded-lg border" disabled={isConnected} />
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input type="text" value={tags} onChange={(e) => setTags(e.target.value)} placeholder="tag1, tag2, tag3" className="w-full p-2 rounded-lg border" disabled={!isConnected || uploading} />
                </div>
                <div>
                ...
                <button onClick={handleUpload} disabled={!isConnected || uploading || !videoFile || !title} className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-2 rounded-lg transition flex items-center justify-center gap-2 disabled:opacity-50">
                ...
                {uploading && (
                  <div>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-red-500 h-2 rounded-full" style={{ width: `${uploadProgress}%` }}></div></div>
                  </div>
                )}
              </div>
            </div>
