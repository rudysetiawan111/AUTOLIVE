// frontend/app/dashboard/features/tiktok-downloader/page.tsx
'use client';
...
import { 
  ...
  Music, Video, Loader2, CheckCircle, AlertCircle, 
  ExternalLink, History, Trash2, Search, Clock, Hash, Copy, Link as LinkIcon
} from 'lucide-react';
...
export default function TikTokDownloaderPage() {
  ...
  const fetchHistory = async () => {
    ...
    setHistory([
      { id: '1', title: 'TikTok Viral Dance', quality: '1080p', format: 'mp4', date: new Date().toISOString(), status: 'success', size: 28.5, url: 'https://tiktok.com/@user/video/abc' },
      { id: '2', title: 'Funny Cat Compilation', quality: '720p', format: 'mp4', date: new Date(Date.now() - 86400000).toISOString(), status: 'success', size: 22.3, url: 'https://tiktok.com/@user/video/def' },
      { id: '3', title: 'Music Remix', quality: 'mp3', format: 'mp3', date: new Date(Date.now() - 172800000).toISOString(), status: 'failed', error: 'No watermark option failed', url: 'https://tiktok.com/@user/video/ghi' },
    ]);
    setFilteredHistory(history);
  };
  ...
  const fetchVideoInfo = async () => {
    if (!url.trim()) {
      toast.error(language === 'en' ? 'Please enter a TikTok URL' : 'Masukkan URL TikTok');
      return;
    }
    ...
    setVideoInfo({
      ...
      duration: '0:45',
      thumbnail: 'https://via.placeholder.com/120x68?text=TT',
      formats: [
        { quality: '720p', size: 15, format: 'mp4', withWatermark: true },
        { quality: '720p', size: 12, format: 'mp4', withWatermark: false },
        { quality: '480p', size: 10, format: 'mp4', withWatermark: true },
        { quality: '480p', size: 8, format: 'mp4', withWatermark: false },
        { quality: 'mp3', size: 3, format: 'mp3', withWatermark: false },
      ]
    });
      ...
      setDownloadProgress(prev => {
        if (prev >= 100) {
          ...
          const newItem = {
            ...
            status: 'success',
            size: quality === '1080p' ? 28 : quality === '720p' ? 15 : quality === '480p' ? 10 : 3,
            url: url
          };
          setHistory(prev => [newItem, ...prev]);
          return 100;
        }
        const speed = (Math.random() * 1.5 + 0.3).toFixed(1);
  ...
  const t = (key: string) => {
    const translations: any = {
      en: {
        pageTitle: 'AUTOLIVE TIKTOK DOWNLOADER',
        subTitle: 'DOWNLOAD FROM TIKTOK',
        ...
        videoUrl: 'TikTok URL',
        urlPlaceholder: 'Enter TikTok URL (e.g., https://tiktok.com/@user/video/...)',
        fetchInfo: 'Get Video Info',
        videoInfo: 'Video Info',
        title: 'Title',
        author: 'Author',
        duration: 'Duration',
        quality: 'Quality / Watermark',
        format: 'Format',
      ...
      id: {
        ...
        downloadForm: 'Download Video',
        videoUrl: 'URL TikTok',
        urlPlaceholder: 'Masukkan URL TikTok (contoh: https://tiktok.com/@user/video/...)',
        fetchInfo: 'Ambil Info Video',
        videoInfo: 'Info Video',
        title: 'Judul',
        author: 'Pembuat',
        duration: 'Durasi',
        quality: 'Kualitas / Watermark',
        format: 'Format',
        ...
        mb: 'MB',
        withWatermark: 'Dengan watermark',
        withoutWatermark: 'Tanpa watermark',
      }
    };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
              <h3 className="text-lg font-semibold mb-4 flex items-center gap-2"><Download className="w-5 h-5 text-black dark:text-white" /> {t('downloadForm')}</h3>
                  ...
                  <div className="border rounded-lg p-3 space-y-2">
                    <div className="flex gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{videoInfo.author} • {videoInfo.duration}</p>
                      </div>
                    </div>
                      ...
                      <div>
                        ...
                        <select value={quality} onChange={(e) => setQuality(e.target.value)} className="w-full p-1 border rounded text-sm">
                          {videoInfo.formats.map((f: any) => (
                            <option key={f.quality + (f.withWatermark ? 'wm' : 'nowm')} value={f.quality}>
                              {f.quality} {f.withWatermark ? t('withWatermark') : t('withoutWatermark')}
                            </option>
                          ))}
                        </select>
                      </div>
                    ...
                    <button onClick={startDownload} disabled={downloading} className="w-full bg-black dark:bg-white dark:text-black hover:bg-gray-800 text-white font-semibold py-2 rounded-lg flex items-center justify-center gap-2 disabled:opacity-50">
                      {downloading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Music className="w-4 h-4" />}
                    ...
                    {downloading && (
                      <div className="space-y-1">
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-1.5"><div className="bg-black dark:bg-white h-1.5 rounded-full" style={{ width: `${downloadProgress}%` }}></div></div>
                      </div>
                    )}
                  </div>
                )}
